...
- **ПК и мобильные устройства.** Отдельное управление мышью и клавиатурой, отдельный сенсорный интерфейс: стик, свайп и экранные кнопки.
...
`npm test` — быстрая проверка без браузера: ассеты, совпадение разметки с кодом, баланс всех 30 уровней (включая запас топлива под цель, товары витрины и терпение спешащих), таблица достижений и миграция сохранений.
...
Полноэкранная реклама показывается только между сменами. Rewarded-реклама доступна добровольно на экране результатов и даёт надбавку **35% от заработка смены, но не меньше ₽50**; кнопка подписана точной суммой, и эта сумма не меняется от того, что уже выкуплено на станции.
