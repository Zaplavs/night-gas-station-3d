...
const music=await page.evaluate(async()=>{const {AudioSystem}=await import('/src/audio.js'),a=new AudioSystem();a.ensure();const result={procedural:!!a.music&&a.music.voices.length===4&&a.music.chords.length===4,melodic:a.music?.melodies.every(notes=>notes.length===3),louder:a.music?.bus.gain.value>.25,noExternalTrack:!a.music?.audioElement,muteWorks:false,volumeWorks:false};a.setMusicVolume(.35);result.volumeWorks=a.musicVolume===.35&&!!a.music.lfoGain;a.setMuted(true);result.muteWorks=a.muted===true;clearInterval(a.musicTimer);await a.ctx?.close();return result});
...
const chores=await page.evaluate(()=>{const g=window.__nightStation;
  ...
  {const mop=g.jobs.find(j=>j.tag==='stand-mop').pos();g.player.position.set(mop.x+1,.26,mop.z);g.updateInteraction(0)}
...
console.log('E2E passed: night sky and jumping, both routes behind the counter, indoor mop, music controls, doors, blackout, fuel flow and traffic are working.');await browser.close();
