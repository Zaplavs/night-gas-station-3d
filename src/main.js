...
class NightStationGame{
  ...
  updateDoors(dt){if(!this.player)return;const nearby=Math.abs(this.player.position.x)<2&&Math.abs(this.player.position.z+2.65)<2.35,target=this.blackout||nearby?1:0,speed=2.8*dt;this.doorOpen=THREE.MathUtils.clamp(this.doorOpen+(target>this.doorOpen?speed:-speed),0,1);this.applyDoorOpen()}
  updatePlayer(dt){
    ...
    this.camera.position.set(this.player.position.x,EYE_HEIGHT+bob+this.jumpHeight-this.landDip,this.player.position.z);
  ...
  updateInteraction(dt){
    let nearest=null,best=2.15;for(const j of this.jobs){const p=j.pos();let d=Math.hypot(p.x-this.player.position.x,p.z-this.player.position.z);if(j.hidden)d+=.85;if(j.need&&this.carry===j.need)d-=.5;if(d<best){best=d;nearest=j}}
  ...
  setBlackout(v){this.stationLights.forEach((l,i)=>l.intensity=v?0:i===2?22:28);this.hemi.intensity=v?.45:1.85;this.powerVisuals.forEach(o=>o.visible=!v);if(v){this.doorOpen=1;this.applyDoorOpen()}}
