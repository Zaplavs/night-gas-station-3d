...
class NightStationGame{
  constructor(){
    ...
    this.clock=new THREE.Clock();this.assets={};this.mode='loading';this.player=null;this.jobs=[];this.cars=[];this.pumps=[];this.nearest=null;this.actionHeld=false;this.actionProgress=0;this.keys={};this.joy={x:0,y:0};this.yaw=0;this.pitch=-.04;this.lookTouch=null;this.jobId=0;this.taskSignature='';this.guideIndex=0;this.resumeAfterGuide=false;this.elapsed=0;this.shiftLength=210;this.spawnTimer=2;this.eventTimer=24;this.stock=6;this.carry=null;this.served=0;this.shiftStartMoney=0;this.blackout=false;this.specialVan=null;this.state={...SAVE_DEFAULT,upgrades:{...SAVE_DEFAULT.upgrades}};
  ...
  setupWorld(){
    ...
    const ground=new THREE.Mesh(new THREE.PlaneGeometry(90,90),new THREE.MeshStandardMaterial({color:0x182620,roughness:1}));ground.rotation.x=-Math.PI/2;ground.receiveShadow=true;this.scene.add(ground);
    ...
    const road=new THREE.Mesh(new THREE.PlaneGeometry(70,9),new THREE.MeshStandardMaterial({color:0x070b0e,roughness:.97}));road.rotation.x=-Math.PI/2;road.position.set(0,.02,-17);road.receiveShadow=true;this.scene.add(road);
    const stripeMat=new THREE.MeshBasicMaterial({color:0xb7a55f});for(let x=-32;x<33;x+=6){const s=new THREE.Mesh(new THREE.PlaneGeometry(3.2,.13),stripeMat);s.rotation.x=-Math.PI/2;s.position.set(x,.035,-17);this.scene.add(s)}
  ...
  async load(){
    ...
    for(const x of [-2.55,2.55]){const model=this.assets.pump.clone(true);model.position.set(x,.24,-7.4);this.scene.add(model);this.pumps.push({x,z:-7.4,model,car:null,broken:false})}
    ...
    const kit=this.assets.cleaning_kit.clone(true);kit.position.set(-3.7,.25,-3.25);kit.scale.setScalar(.8);this.scene.add(kit);
  ...
  setupInput(){
    ...
    addEventListener('keyup',e=>{this.keys[e.code]=false;if(e.code==='KeyE'){this.actionHeld=false;this.actionProgress=0}});addEventListener('blur',()=>{this.keys={};this.actionHeld=false;this.joy={x:0,y:0}});
    ...
    const act=$('#action-btn');act.addEventListener('pointerdown',e=>{e.preventDefault();this.actionHeld=true;act.setPointerCapture(e.pointerId)});const release=()=>{this.actionHeld=false;this.actionProgress=0};act.addEventListener('pointerup',release);act.addEventListener('pointercancel',release);
  }
  ...
  renderGuide(){const slides=[
    ...
    ['Колонки и машины','Две красные колонки стоят под вывеской перед магазином. Машины останавливаются сбоку от них. Подойдите к отмеченной машине и удерживайте E.','tutorial/02-pumps.png'],
    ['Кофе, еда и склад','Кофемашина — слева на прилавке, еда — на стеллаже справа. Коробки для пополнения находятся в дальнем правом углу магазина.','tutorial/03-shop.png'],
    ['Ночные происшествия','Список задач всегда слева. Щиток находится в дальнем левом углу, бюро находок — справа. Оранжевый луч показывает точное место действия.','tutorial/04-events.png']
  ...
  updatePlayer(dt){
    ...
    if(len>.05){strafe/=Math.max(1,len);forward/=Math.max(1,len);const run=this.keys.ShiftLeft||this.keys.ShiftRight?1.55:1,speed=(4.35+this.state.upgrades.speed*.52)*run,fx=-Math.sin(this.yaw),fz=-Math.cos(this.yaw),rx=Math.cos(this.yaw),rz=-Math.sin(this.yaw),dx=(rx*strafe+fx*forward)*speed*dt,dz=(rz*strafe+fz*forward)*speed*dt;const nextX=this.player.position.x+dx,nextZ=this.player.position.z+dz;if(!this.isBlocked(nextX,this.player.position.z))this.player.position.x=nextX;if(!this.isBlocked(this.player.position.x,nextZ))this.player.position.z=nextZ;moving=true}
    const bob=moving?Math.sin(this.elapsed*11)*.035:0;this.camera.position.set(this.player.position.x,1.62+bob,this.player.position.z);this.camera.rotation.set(this.pitch,this.yaw,0,'YXZ')
  }
  isBlocked(x,z){
  ...
  updateCars(dt){
    for(const c of [...this.cars]){
      if(c.status==='arriving'){c.t=Math.min(1,c.t+dt*.3);const e=1-Math.pow(1-c.t,3);c.group.position.lerpVectors(c.start,c.target,e);if(c.t>=1){c.status='fuel';this.addFuelJob(c)}}
      else if(c.status==='leaving'){c.t=Math.min(1,c.t+dt*.18);c.group.position.lerpVectors(c.start,c.target,c.t*c.t);if(c.t>=1){this.scene.remove(c.group);this.cars.splice(this.cars.indexOf(c),1);if(c.pump)c.pump.car=null}}
    }
    if(this.specialVan){const v=this.specialVan;if(v.status==='arriving'){v.t=Math.min(1,v.t+dt*.1);v.group.position.lerpVectors(v.start,v.target,1-Math.pow(1-v.t,3));if(v.t>=1){v.status='waiting';this.addJob({title:'Проверьте странный фургон',sub:'Водитель молчит и не открывает окно',pos:()=>v.group.position,duration:2.8,patience:40,onComplete:()=>{this.state.money+=90;this.state.rep=Math.min(5,this.state.rep+.1);this.eventNotice('…','Внутри никого','На сиденье лежал чек с завтрашней датой. Вы получили ₽90.');audio.spooky();this.leaveSpecialVan()}})}}else if(v.status==='leaving'){v.t=Math.min(1,v.t+dt*.12);v.group.position.lerpVectors(v.start,v.target,v.t*v.t);if(v.t>=1){this.scene.remove(v.group);this.specialVan=null}}}
  }
  spawnCar(){
    const pump=this.pumps.find(p=>!p.car&&!p.broken);if(!pump)return;const group=this.assets.car.clone(true),colors=[0xb52f28,0x2e6380,0xd0a343,0x4f555b,0x315d3f],color=colors[Math.floor(Math.random()*colors.length)],laneX=pump.x+Math.sign(pump.x)*2.05;group.traverse(o=>{if(o.isMesh&&o.name==='CarBody'){o.material=o.material.clone();o.material.color.setHex(color)}});group.position.set(laneX,-.02,-20);group.rotation.y=0;this.scene.add(group);const c={group,pump,status:'arriving',t:0,start:group.position.clone(),target:new THREE.Vector3(laneX,.25,pump.z),patience:46-Math.min(12,this.state.shift*1.2)};pump.car=c;this.cars.push(c);audio.tone(105,.35,'sawtooth',.12)
  }
  addFuelJob(car){
    car.job=this.addJob({title:'Заправьте машину',sub:`Колонка ${this.pumps.indexOf(car.pump)+1} · бензин АИ-95`,pos:()=>car.group.position,duration:2.7,patience:car.patience,onFail:()=>this.loseCustomer(car),onComplete:()=>{const gain=75+this.state.shift*4;this.state.money+=gain;this.state.rep=Math.min(5,this.state.rep+.04);audio.success();this.toast(`Полный бак <b>+₽${gain}</b>`);if(Math.random()<.62)this.createOrder(car);else{this.served++;setTimeout(()=>this.leaveCar(car),800)}}})
  }
  createOrder(car){
  ...
  renderTasks(){const shown=this.jobs.slice(0,5),signature=shown.map(j=>j.id).join(',')+'|'+(this.nearest?.id||0);if(signature!==this.taskSignature){this.taskSignature=signature;ui.taskList.innerHTML=shown.map((j,i)=>`<div class="task ${j===this.nearest?'active':''}" data-job="${j.id}"><span class="num">0${i+1}</span><b>${j.title}</b><small>${j.sub||''}</small>${j.patience!=null?'<div class="patience"><i></i></div>':''}</div>`).join('')||'<div class="task"><span class="num">✓</span><b>Всё спокойно</b><small>Осмотритесь вокруг</small></div>'}for(const j of shown){if(j.patience==null)continue;const bar=ui.taskList.querySelector(`[data-job="${j.id}"] .patience i`);if(bar)bar.style.width=`${Math.max(0,j.patience/j.maxPatience*100)}%`}}
  updateInteraction(dt){
    ...
    this.marker.visible=!!nearest;if(nearest){const p=nearest.pos();this.marker.position.x=p.x;this.marker.position.z=p.z;ui.prompt.classList.remove('hidden');ui.promptKey.textContent=isTouch?'●':'E';ui.promptTitle.textContent=nearest.title;ui.promptSub.textContent='Удерживайте для действия'}else ui.prompt.classList.add('hidden');
    if(nearest&&this.actionHeld){this.actionProgress+=dt*(1+this.state.upgrades.service*.15);ui.progress.classList.remove('hidden');ui.progressFill.style.width=`${Math.min(100,this.actionProgress/nearest.duration*100)}%`;if(this.actionProgress>=nearest.duration){this.actionProgress=0;const ok=nearest.onComplete?.();if(ok!==false)this.removeJob(nearest)}}else{this.actionProgress=Math.max(0,this.actionProgress-dt*2.5);ui.progress.classList.add('hidden')}
  }
  triggerEvent(){
    const options=['spill','blackout','bag','broken','van','whisper'].filter(x=>!(x==='blackout'&&this.blackout)&&!(x==='van'&&this.specialVan)&&!(x==='bag'&&this.jobs.some(j=>j.tag==='bag')));const type=options[Math.floor(Math.random()*options.length)];
  ...
  eventBag(){const group=this.assets.bag.clone(true),p=new THREE.Vector3(-.8,.16,-3.25);group.position.copy(p);this.scene.add(group);this.eventNotice('?','Забытая сумка','Хозяина не видно. Лучше убрать её в бюро находок.');this.addJob({tag:'bag',title:'Подберите сумку',sub:'Она появилась у входа',pos:()=>p,duration:1.2,visual:group,onComplete:()=>{if(this.carry)return false;this.carry='bag';this.updateCarry();this.addJob({tag:'bag',title:'Отнесите сумку',sub:'В ящик бюро находок',pos:()=>new THREE.Vector3(4.1,.2,4.1),duration:1,onComplete:()=>{if(this.carry!=='bag')return false;this.carry=null;this.updateCarry();this.state.rep=Math.min(5,this.state.rep+.18);this.toast('Честность замечена <b>+репутация</b>');audio.success()}})}})}
  eventBrokenPump(){const p=this.pumps[Math.floor(Math.random()*this.pumps.length)];if(p.broken||p.car){this.spawnSpill();return}p.broken=true;this.eventNotice('⚙','Заклинило колонку','Пахнет проводкой, но искр пока нет. Нужен инструмент.');this.addJob({title:`Почините колонку ${this.pumps.indexOf(p)+1}`,sub:'Подтяните контакт и перезапустите насос',pos:()=>new THREE.Vector3(p.x,.2,p.z),duration:3.1,onComplete:()=>{p.broken=false;this.state.money+=65;this.toast('Колонка снова работает <b>+₽65</b>');audio.success()}})}
  eventVan(){const group=this.assets.mystery_van.clone(true);group.position.set(8,.05,-20);this.scene.add(group);this.specialVan={group,status:'arriving',t:0,start:group.position.clone(),target:new THREE.Vector3(7,.25,-8)};this.eventNotice('◉','Подозрительный фургон','Он подъехал без фар. Номер заляпан чем-то похожим на пепел.');audio.spooky()}
  leaveSpecialVan(){const v=this.specialVan;if(!v)return;v.status='leaving';v.t=0;v.start=v.group.position.clone();v.target=new THREE.Vector3(22,.05,-18)}
  ...
  updateCarry(){const names={coffee:'Кофе',snack:'Сэндвич',box:'Коробка товара',bag:'Забытая сумка'};ui.carrying.classList.toggle('hidden',!this.carry);$('#carrying b').textContent=names[this.carry]||''}
