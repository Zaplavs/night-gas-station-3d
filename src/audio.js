export class AudioSystem {
  constructor(){this.ctx=null;this.master=null;this.muted=false;this.ambience=null;this.music=null;this.musicTimer=null}
  ensure(){
    if(this.ctx)return;
  ...
  startMusic(){
    ...
    const bus=this.ctx.createGain(),filter=this.ctx.createBiquadFilter(),lfo=this.ctx.createOscillator(),lfoGain=this.ctx.createGain();bus.gain.value=.105;filter.type='lowpass';filter.frequency.value=850;filter.Q.value=.65;bus.connect(filter);filter.connect(this.master);lfo.type='sine';lfo.frequency.value=.045;lfoGain.gain.value=.018;lfo.connect(lfoGain);lfoGain.connect(bus.gain);lfo.start();
    ...
    this.music={bus,voices,step:0,chords:[[73.42,110,146.83,174.61],[58.27,116.54,146.83,174.61],[87.31,130.81,174.61,220],[65.41,98,130.81,196]],melodies:[[0,2,1],[2,1,3],[1,3,2],[2,0,1]]};
  ...
  musicNote(frequency,start,duration){
    const oscillator=this.ctx.createOscillator(),gain=this.ctx.createGain();oscillator.type='triangle';oscillator.frequency.setValueAtTime(frequency,start);gain.gain.setValueAtTime(.0001,start);gain.gain.exponentialRampToValueAtTime(.055,start+.12);gain.gain.exponentialRampToValueAtTime(.0001,start+duration);oscillator.connect(gain);gain.connect(this.music.bus);oscillator.start(start);oscillator.stop(start+duration+.05);
  }
}
